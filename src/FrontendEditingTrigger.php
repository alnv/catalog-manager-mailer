...
class FrontendEditingTrigger extends CatalogController
{
    ...
    public function onCreate($arrData, $objModule)
    {
        ...
        $intMailerId = $objModule->catalogMailerInsert ?: $objModule->catalogMailerDuplicate;
    ...
    protected function runMailer($intMailerId)
    {
        ...
        $objMailer = Database::getInstance()->prepare('SELECT * FROM tl_mailer WHERE id = ?')->limit(1)->execute($intMailerId);
    ...
    protected function setPostData($strType, $arrData): void
    {
        ...
        if (\in_array($strType, ['duplicate', 'update'])) {
            ...
            if ($arrData['id']) {
                ...
                $objEntity = Database::getInstance()->prepare(sprintf('SELECT * FROM %s WHERE id = ?', $arrData['table']))->limit(1)->execute($arrData['id']);
